...
//create browser context with edge and maximized window
async function createContextWithEdgeAndMaximizedWindow(browser: Browser) {
...
test.describe('Search + Listing Validation', () => {
  ...
  test('search returns valid list entries', async ({ page, browser }) => {
    const context = await createContextWithEdgeAndMaximizedWindow(browser);
    const newPage = await context.newPage();
    try {
      await navigateToHome(newPage);
      await newPage.waitForTimeout(5000);
      const acceptBtn = await getAcceptAllBtn(newPage);
      if (acceptBtn) {
        await acceptBtn.click({ timeout: 2000 });
        console.log("clicked accept all")
      }
      const startSearchBtn = await getStartSearchBtn(newPage);
      if (startSearchBtn) {
        await startSearchBtn.nth(0).click({ timeout: 3000 });
      }
      await performSearch(newPage, 'Berlin');
      await scrollToItemOrItems(newPage, 1);
      await newPage.waitForTimeout(5000);
      await expectListingCount(newPage, 4);

      await expectItemsContainCity(newPage, 'Berlin');
    } catch (error) {
  ...
  test.only('unknown city or zip code returns no listing', async ({ page }) => {
    try {
