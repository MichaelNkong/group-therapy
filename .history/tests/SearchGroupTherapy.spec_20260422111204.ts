...
async function getFirstResultText(page: Page): Promise<string> {
  const items = getListingItems(page);
  return await items.nth().innerText();
}
...
test.describe('Search + Listing Validation', () => {
  ...
  test.only('Search results updates when changing city', async ({ page }) => {
    try {
      await navigateToHome(page);
      await page.waitForTimeout(5000);
       await page.waitForTimeout(5000);
  ...
  test('search page displays default data within acceptable loading time', async ({ page }) => {
    try {
      ...
      await startSearchBtn.nth(0).click({ timeout: 3000 });
      //wait until the network is idle to ensure all data has loaded 
      ...
      expect(loadingTime).toBeLessThan(2000); // Expect loading time to be less than 2 seconds
    } catch (error) {
      console.error('Test failed:', error);
      throw error;
    }
