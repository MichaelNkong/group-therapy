...
async function getAcceptAllBtn(page: Page): Promise<Locator> {
  const input = page.getByRole('button', { name: 'Alle akzeptieren' });
  await expect(input).toBeVisible();
  return input;
}
...
async function getStartSearchBtn(page: Page): Promise<Locator> {
  const input = page.getByRole('button', { name: 'Suche Starten' });
  await expect(input).toBeVisible();
  return input;
}
...
function getListingItems(page: Page): Locator {
  return page.locator('[data-container="map-list-wrapper OSInline"] .map-list-item', {timeout: 5000});
}
