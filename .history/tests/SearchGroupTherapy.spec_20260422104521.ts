...
test.describe('Search + Listing Validation', () => {
  ...
  test('search page displays data within acceptable loading time', async ({ page }) => {
    try {
      ...
      await startSearchBtn.nth(0).click({ timeout: 3000 });
      //wait until the network is idle to ensure all data has loaded (you can adjust the timeout as needed)
        await page.waitForLoadState('networkidle', { timeout: 15000 });
      // Wait for the search results to load you can adjust the selector and timeout as needed)
