...
test.describe('Search + Listing Validation', () => {
  ...
  test.only('search page displays default data within acceptable loading time', async ({ page }) => {
    try {
      await page.goto(`${BASE_URL}`);
      await navigateToHome(page);
      await page.waitForTimeout(5000);
  ...
  //  wait for actual data items
    await page.waitForSelector('.map-list.list.list-group.display-flex.flex-direction-column.OSFillParent > div[data-container]', { state: 'visible', timeout: 15000 });
