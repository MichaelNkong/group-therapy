...
async function scrollToThirdItem(page: Page, position: number = 2) {
  await page.locator('.map-list > div[data-container]').nth(position).scrollIntoViewIfNeeded();
}
/**
...
test.describe('Search + Listing Validation', () => {
  ...
  test.only('search returns valid list entries', async ({ page }) => {
    try {
      ...
      await scrollToThirdItem(page);
      await page.waitForTimeout(5000);
      await expectListingCount(page, 3);
